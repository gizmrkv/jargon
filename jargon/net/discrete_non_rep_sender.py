...
class DiscreteNonRepSender(nn.Module):
    def __init__(
        self,
        num_elems: int,
        num_attrs: int,
    ...
    def forward(
        ...
        if message is None:
            ...
            for _ in range(self.max_len):
                ...
                logits = logits_step.detach().clone()
                if len(symbol_list) > 0:
                    prev_symbol = symbol_list[-1]
                    prev_symbol = torch.nn.functional.one_hot(
                        prev_symbol, num_classes=self.vocab_size
                    )
                    prev_symbol = prev_symbol == 1
                    prev_symbol[:, :, 0] = False
                    prev_symbol[:, :, 1] = False
                    logits[prev_symbol] = float("-inf")
